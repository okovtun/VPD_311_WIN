...
TODO:
1. Параметры программы должны схраняться при выходе, и загружаться при старте;				DONE
2. Включить двойную буфферизацию, это уберет мерцание;										DONE
3. В контекстное меню добавить пункт 'Alarms', который будет откывать окно с будильниками;	DONE
4. Должна быть возможность добавлять, удалять и изменять будильники;						DONE
5. Будильники должны звонить в свое время;													DONE
6. Когда будильник доиграл, или когда пользователь нажал кнопку Stop плейер должен скрываться;
7. При срабатывании будильника преусмотреть дату и дни недели;
8. При выходе из программы будильники должны сохраняться, и при загрузке программы они должны снова загружаться;
9. Часы должны перемещаться мышкой даже когда не видны элементы управления;

DONE:
1. Реализовать элементы контекстного меню;						DONE
	Topmost - часы будут отображаться поверх всех окон;
	Show controls - показать элементы управления;
2. Прикрутить кастомный шрифт;									DONE
